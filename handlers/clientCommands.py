...
def startListen(bot: telebot.TeleBot, botLogger: logging.Logger):
    ...
    def regClientGen(msg: telebot.types.Message, client=(None, None, None)):
        if client[0] is None:
            reply = bot.send_message(msg.chat.id, Replicas.ASK_NAME_CLIENT)
            msg: telebot.types.Message = yield reply, False
            client = (msg.text, client[1], client[2])
            bot.send_message(msg.chat.id, Replicas.ON_GET_NAME + ', ' + msg.text + ' ' + Emoji.HANDSHAKE)
        if client[1] is None:
        ...
        reply = bot.send_message(msg.chat.id, Replicas.ON_REGISTRATION_CLIENT,
    ...
    def changeClientPoint(msg: telebot.types.Message):
        ...
        if client is None:
            bot.send_message(msg.chat.id, Replicas.WELCOME_CLIENT)
            return
